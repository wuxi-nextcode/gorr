...
```{r}
name <- "Diabetes" # Name of new playlist
...
Once a playlist has been fetched or created we can add phenotypes to it. As for adding phenotypes when createing a new playlist we need to make sure that the phenotype does exist in the catalog and has not been assinged to the playlist alread.

```{r, eval=FALSE}
phenos <- get_phenotypes(conn, limit = 2) # Fetch two existing phenotypes
print(phenos)
new_playlist  <- playlist_add_phenotype(phenos, new_playlist, conn)
print(new_playlist$phenotypes)
```
